...
var Line = function (_Group) {
  ...
  function Line(data, lines, axisConfig) {
    ...
    lines.forEach(function (item) {
      ...
      _this.add(new OneLine(item, data));
    });
    return _this;
  }
...
var OneLine = function (_Group2) {
  ...
  function OneLine(option, data) {
    ...
    if (option.smooth) {
      ...
      To.get(py).to({ py: option.projectionY }, 2000, option.show.easing).progress(function (object) {
        update(py.py, path, g, pg, option);
      }).end(function () {
        var cg = createCircles(option.endPoints, option);
        _this2.add(cg);
        fadeIn(cg);
      }).start();
    } else {
      var _g = new Graphics();
      _g.beginPath().strokeStyle(option.color);
      path.forEach(function (p, index) {
        _g[index === 0 ? 'moveTo' : 'lineTo'](p.x, p.y);
      });
      _g.stroke();

      var _pg = new Graphics();
      _pg.alpha = 0.5;
      if (option.hasOwnProperty('projectionY')) {
        _pg.beginPath().fillStyle(option.color);
        path.forEach(function (p, index) {
          _pg[index === 0 ? 'moveTo' : 'lineTo'](p.x, p.y);
        });

        _pg.lineTo(path[path.length - 1].x, 0);
        _pg.lineTo(path[0].x, 0);
        _pg.closePath();
        _pg.fill();
      }
      _pg.x = option.x;
      ...
      var _py = { py: 0 };
      path.forEach(function (p, index) {
        new TWEEN.Tween(p).to(option.endPoints[index], 2000).delay(index * 50).easing(option.show.easing || TWEEN.Easing.Linear.None).onUpdate(function (object) {
          _g.clear().beginPath().strokeStyle(option.color);
          path.forEach(function (p, index) {
            _g[index === 0 ? 'moveTo' : 'lineTo'](p.x, p.y);
          });
          _g.stroke();
      ...
      new TWEEN.Tween(_py).to({ py: option.projectionY }, 2000).easing(option.show.easing || TWEEN.Easing.Linear.None).onUpdate(function (object) {
        if (option.hasOwnProperty('projectionY')) {
          _pg.clear().beginPath().fillStyle(option.color);
          path.forEach(function (p, index) {
            _pg[index === 0 ? 'moveTo' : 'lineTo'](p.x, p.y);
          });

          _pg.lineTo(path[path.length - 1].x, _py.py);
          _pg.lineTo(path[0].x, _py.py);
          _pg.closePath();
          _pg.fill();
        }
      }).onComplete(function () {
      ...
      case 'top':
        for (var _i2 = f; _i2 <= t; _i2 += axis.interval) {
          ...
          if (!axis.text.range || _i2 >= axis.text.range[0] && _i2 <= axis.text.range[1]) {
            ...
            _text2.x = current + axis.text.x - _text2.getWidth() / 2;
