...
var Line = function (_Group) {
  ...
  function Line(data) {
    ...
    var twoAxis = false;
    data.datasets.every(function (ds) {
      console.log(ds);
      if (ds.right) {
        console.log(2);
        twoAxis = ds.right;
    ...
    var bottomY = data.height - 30;
    var startX = 100;
    var xScale = (0, _scale.scaleLinear)([0, data.labels.length], [startX, data.width]);
    var xInterval = (data.width - startX) / data.labels.length;
    ...
    var lmin = data.datasets[0].data[0];
    var lmax = lmin;
    if (!twoAxis) {
    ...
    var interval = (lmax - lmin) / 8;
    var yScaleLeft = (0, _scale.scaleLinear)([lmin - interval, lmax + interval], [bottomY, 0 + 30]);
    ...
    var yScaleRight = void 0;

    if (twoAxis) {
      console.log(1);
      var rmin = Math.min.apply(null, data.datasets[1].data);
      var rmax = Math.max.apply(null, data.datasets[1].data);
      var rinterval = (rmax - rmin) / 8;
      yScaleRight = (0, _scale.scaleLinear)([rmin - rinterval, rmax + rinterval], [bottomY, 0 + 30]);
      axisConfig.right = {
        scale: yScaleRight,
        color: 'black',
        interval: rinterval,
    ...
    data.datasets.forEach(function (dataset, index) {
      ...
      _this.add(new OneLine(dataset, yScaleLeft, xInterval, startX, yScaleRight));
    });
...
var OneLine = function (_Group2) {
  _inherits(OneLine, _Group2);

  function OneLine(dataset, yScaleLeft, xInterval, startX, yScaleRight) {
    _classCallCheck(this, OneLine);

    var _this2 = _possibleConstructorReturn(this, (OneLine.__proto__ || Object.getPrototypeOf(OneLine)).call(this));

    var eps = [];
    dataset.data.forEach(function (value, index) {
      eps.push({ x: xInterval * index + startX + xInterval / 2, y: dataset.right ? yScaleRight(value) : yScaleLeft(value), value: value });
    });

    _this2.add(createLine(eps, dataset.color));
    _this2.add(createCircles(eps, dataset.color));
    return _this2;
  }
...
function createCircles(eps, color) {
  var group = new Group();
  eps.forEach(function (ep, index) {
    var circle = new Circle(3, { lineWidth: 2, fillStyle: 'white', strokeStyle: color });

    circle.x = ep.x;
    circle.y = ep.y;
    var text = new _cax2.default.Text(ep.value, { color: color, font: '14px Arial' });
...
function hideLine(group, options, callback) {
  var cpt = false;
  ...
  group.children.forEach(function (subGroup, index) {
    ...
    subGroup.children.forEach(function (rect, rectIndex) {
      new TWEEN.Tween(from).to(to, option.hide.duration).delay(typeof option.hide.delay === 'function' ? option.hide.delay(rectIndex) : option.hide.delay || 0).easing(option.hide.easing || TWEEN.Easing.Linear.None).onUpdate(function (object) {
...
function update(projectionY, path, g, pg, option) {
  var cps = (0, _getControlPoint.getControlPoint)(path);
