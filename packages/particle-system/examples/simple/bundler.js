...
var ParticleSystem = function (_cax$Group) {
  _inherits(ParticleSystem, _cax$Group);

  function ParticleSystem(option) {
    _classCallCheck(this, ParticleSystem);

    var _this = _possibleConstructorReturn(this, (ParticleSystem.__proto__ || Object.getPrototypeOf(ParticleSystem)).call(this));

    _this.speedRange = option.speedRange;
    _this.angleRange = option.angleRange;
    _this.emitRange = option.emitRange; //支持圆形和方形
    _this.gravity = option.gravity || { x: 0, y: 0 };
    _this.filter = option.filter;

    _this.emitCount = option.emitCount || 1;
    _this.maxCount = option.maxCount || 100;
    _this.emitX = option.emitX || 0;
    _this.emitY = option.emitY || 0;
    _this.texture = option.texture;
    _this.option = option;
    return _this;
  }
  ...
  _createClass(ParticleSystem, [{
    key: 'emit',
    value: function emit() {
      var angle = randomInt(this.angleRange[0], this.angleRange[1]) * Math.PI / 180;
      var speed = randomInt(this.speedRange[0], this.speedRange[1]);
      var hw = this.emitRange[0] / 2;
      var hh = this.emitRange[1] / 2;
...
var Particle = function (_cax$Group) {
  _inherits(Particle, _cax$Group);
  ...
  function Particle(option) {
    ...
    _this.x = option.x || 0;
    _this.y = option.y || 0;
    _this.rotation = option.rotation || 0;
    _this.velocity = option.velocity;
    _this.acceleration = option.acceleration || 0;
    _this.rotatingSpeed = option.rotatingSpeed || 0;
    _this.rotatingAcceleration = option.rotatingAcceleration || 0;
    _this.hideSpeed = option.hideSpeed || .01;
    _this.zoomSpeed = option.hideSpeed || .01;
    _this.scale = option.scale || 1;
    _this.texture = new _cax2.default.Bitmap(option.texture);
    _this.texture.originX = option.texture.width / 2;
    _this.texture.originY = option.texture.height / 2;
    _this.add(_this.texture);
    _this.fixed = true;
    _this.scaleX = _this.scaleY = option.scale;
    return _this;
  }
