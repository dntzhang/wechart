...
var stage = new _cax2.default.Stage(640, 400, 'body');
...
var Pie = function (_Group) {
  _inherits(Pie, _Group);
      ...
      To.get({ totalAngle: from }).to({ totalAngle: to }, duration, easing).progress(function (object) {
        ...
        if (show) {
          ...
          arr.forEach(function (angle, index) {
            ...
            var currentColor = option.color(index);
...
function bounceIn(obj, from, to) {
  from = from || 0;
  obj.from = from;
  To.get(obj).to({ scaleX: to || 1, scaleY: to || 1 }, 300, _cax2.default.easing.bounceOut).start();
}

function bounceOut(obj, from, to) {
  from = from || 1;
  obj.from = from;
  To.get(obj).to({ scaleX: to || 0, scaleY: to || 0 }, 300, _cax2.default.easing.bounceOut).start();
}
